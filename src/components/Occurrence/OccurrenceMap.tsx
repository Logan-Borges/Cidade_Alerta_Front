...
const OccurrenceMap: React.FC<Props> = ({ occurrences, focusedOccurrence, mapFocusKey }) => {
    ...
    return (
        <div className="rounded-2xl overflow-hidden border border-white/10 bg-white/5 mb-6">
            <MapContainer center={center} zoom={13} scrollWheelZoom={false} style={{ height: 420, width: "100%" }}>
                <TileLayer
                ...
                {occurrences.map((occ) =>
                    occ.lat != null && occ.lng != null ? (
                        <Marker
                            ...
                            position={[occ.lat as number, occ.lng as number]}
                            icon={L.divIcon({
                        ...
                        >
                            <Popup>
                                <div style={{ maxWidth: 260 }}>
                                    <h3 style={{ fontWeight: 700 }}>{occ.title ?? occ.titulo ?? "Sem título"}</h3>
                                    {occ.image_url && (
                                        <div style={{ margin: "8px 0" }}>
                                            <img src={occ.image_url} alt={occ.title} style={{ width: "100%", borderRadius: 8 }} />
                                        </div>
                                    )}
                                    <div style={{ fontSize: 13, color: "#e5e7eb" }}>{occ.description ?? occ.descricao}</div>
                                    <div style={{ marginTop: 8, fontSize: 13 }}>
                                        <strong>Tipo:</strong> {occ.category ?? occ.tipo}
                                    </div>
                                    <div style={{ fontSize: 13 }}>
                                        <strong>Urgência:</strong> {occ.urgency ?? occ.urgencia}
                                    </div>
                                    <div style={{ fontSize: 13 }}>
