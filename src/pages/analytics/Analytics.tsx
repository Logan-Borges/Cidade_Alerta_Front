...
// ── Tipos ────────────────────────────────────────────────────────────────────
type Incident = { titulo?: string; tipo?: string; urgencia?: string; status?: string; bairroId?: number; created_date?: string };
type Item     = { name: string; value: number };

// ── Lookups ──────────────────────────────────────────────────────────────────
const CAT: Record<string,string> = { infraestrutura:"Infraestrutura", seguranca:"Segurança", transito:"Trânsito", saude:"Saúde", meio_ambiente:"Meio Ambiente", outros:"Outros" };
const URG: Record<string,string> = { baixa:"Baixa", media:"Média", alta:"Alta", critica:"Crítica" };
const STA: Record<string,string> = { ativo:"Ativo", em_analise:"Em Análise", em_atendimento:"Em Atendimento", resolvido:"Resolvido" };
const CC = ["#6366f1","#f97316","#06b6d4","#10b981","#f59e0b","#ef4444","#8b5cf6"];
const UC = ["#10b981","#f59e0b","#f97316","#ef4444"];

// ── Helpers ──────────────────────────────────────────────────────────────────
const countBy = (data: Incident[], k: keyof Incident, labels: Record<string,string>): Item[] =>
  Object.entries(data.reduce<Record<string,number>>((a,i) => { const v=(i[k] as string)??"outros"; a[v]=(a[v]??0)+1; return a; }, {}))
    .map(([key,value]) => ({ name: labels[key]??key, value })).sort((a,b)=>b.value-a.value);

// ── Mini-componentes ─────────────────────────────────────────────────────────
const card: React.CSSProperties = { background:"linear-gradient(135deg,#0f1e2e,#162536)", border:"1px solid rgba(255,255,255,.08)", borderRadius:20, padding:24 };

const StatCard = ({ title, value, sub, icon:I, g, d=0 }: { title:string; value:number; sub:string; icon:React.ElementType; g:string; d?:number }) => (
  <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:d,duration:.45,ease:[.16,1,.3,1] as any}}
...
const Card = ({ title, sub, children, style={}, d=0 }: { title:string; sub?:string; children:React.ReactNode; style?:React.CSSProperties; d?:number }) => (
  <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:d,duration:.45,ease:[.16,1,.3,1] as any}} style={{...card,...style}}>
    <div style={{marginBottom:18}}><div style={{fontSize:15,fontWeight:700,color:"#fff"}}>{title}</div>{sub&&<div style={{fontSize:12,color:"rgba(255,255,255,.35)",marginTop:2}}>{sub}</div>}</div>
    {children}
  </motion.div>
);
...
const Donut = ({ data, colors }: { data:Item[]; colors:string[] }) => {
  ...
  const S=150, cx=75, cy=75, IR=40, OR=68, gap=0.05; let a=-Math.PI/2;
...
export default function Analytics() {
  ...
  useEffect(() => {
    svc.getOccurrences<Incident[]>().then(setData).catch(()=>setData([])).finally(()=>setLoading(false));
  }, []);

  const total    = data.length;
  const active   = data.filter(i=>i.status==="ativo").length;
  const resolved = data.filter(i=>i.status==="resolvido").length;
  const critical = data.filter(i=>i.urgencia==="critica").length;

  const catData  = countBy(data,"tipo",CAT);
  const statData = countBy(data,"status",STA);
  const urgData  = countBy(data,"urgencia",URG);

  const bairros = Object.entries(data.reduce<Record<string,number>>((a,i)=>{ const b=String(i.bairroId??"?"); a[b]=(a[b]??0)+1; return a; },{}))
    .map(([name,count])=>({name,count})).sort((a,b)=>b.count-a.count).slice(0,7);

  const timeline = Array.from({length:7},(_,i)=>{ const d=new Date(); d.setDate(d.getDate()-(6-i)); return { day:d.toLocaleDateString("pt-BR",{weekday:"short"}), v:data.filter(o=>o.created_date?.startsWith(d.toISOString().split("T")[0])).length }; });
    ...
    <div style={{minHeight:"100vh",background:"#07101f",color:"#fff"}}>
      ...
      <div style={{maxWidth:1280,margin:"0 auto",padding:"28px 24px",display:"flex",flexDirection:"column",gap:20}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))",gap:14}}>
          <StatCard title="Total" value={total} sub="Todas as categorias" icon={Activity} g="linear-gradient(135deg,#6366f1,#8b5cf6)" d={0}/>
          <StatCard title="Ativos" value={active} sub="Aguardando ação" icon={Clock} g="linear-gradient(135deg,#ef671f,#f97316)" d={.06}/>
          <StatCard title="Críticos" value={critical} sub="Urgência máxima" icon={AlertTriangle} g="linear-gradient(135deg,#ef4444,#f87171)" d={.12}/>
          <StatCard title="Resolvidos" value={resolved} sub="Concluídos com sucesso" icon={CheckCircle2} g="linear-gradient(135deg,#10b981,#34d399)" d={.18}/>
        </div>
        ...
        {/* Bairros */}
        {bairros.length>0&&(
          <Card title="Bairros com Mais Ocorrências" sub={`Top ${bairros.length}`} d={.38}>
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              {bairros.map(({name,count},i)=>{
                  ...
                  <div key={name} style={{display:"flex",alignItems:"center",gap:14}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                        <span style={{fontSize:13,fontWeight:600,color:"rgba(255,255,255,.8)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{name}</span>
                      ...
                      <div style={{height:4,background:"rgba(255,255,255,.07)",borderRadius:4,overflow:"hidden"}}>
                        <motion.div initial={{width:0}} animate={{width:`${Math.round(count/(bairros[0]?.count??1)*100)}%`}}
